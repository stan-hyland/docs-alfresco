...
7.  Set where the cached content is stored, and how much cache size you need.
    ...
    The cached content location (and default value) is `dir.cachedcontent=${dir.root}/cachedcontent`. See [CachingContentStore properties]({% link content-services/7.2/admin/content-stores.md %}#caching-content-store-ccs) for more information on the caching content store.
...
As an existing customer using the default [Encrypted content store]({% link content-services/7.2/admin/content-stores.md %}#encrypted-content-store) configuration, the environment uses:
