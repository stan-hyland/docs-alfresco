...
1. Run a full LDAP sync. This can be done by restarting Alfresco Content Services.
    ...
    > **Note**: If a user exists in LDAP, but not in Alfresco, they will not be able to log in to Alfresco when SAML is enabled. See [Configuring LDAP (Active Directory)]({% link content-services/7.2/admin/auth-sync.md %}#configure-ldap) for more information.
