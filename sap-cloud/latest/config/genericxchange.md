...
| notificationFrequency | String | Defines where in the transaction event the handler is invoked. See the [Behavior Policies Extension Point]({% link content-services/7.2/develop/repo-ext-points/behavior-policies.md %}) documentation for more details. Example value: `TRANSACTION_COMMIT`, `EVERY_EVENT`, or `FIRST_EVENT`. |
